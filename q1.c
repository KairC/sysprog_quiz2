...
bool is_ascii(const char str[], size_t size) {
  if (size == 0)
    return false;
  int i = 0;
  while ((i + 8) <= size) { // check 8 bytes (64-bits) at a time, if the rest of
  ...
  while (i < size) { // because the rest of bytes are insufficient to 8 bytes,
                     // check 1 bytes at a time.
    if (str[i] & 0x80)
      return false;
    i++;
  }
